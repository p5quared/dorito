...
class ContainerConsumerApplication(ConsumerApplication, RedditDataProcessor, SQSStrategy):
    def __init__(self, w: Writer):
        super().__init__(w=w)
...
def app_factory(cfg):
    w = LocalCSVWriter("data.csv", buffer_size=5, fieldnames=get_post_comment_csv_columns())
    if cfg.is_prod:
